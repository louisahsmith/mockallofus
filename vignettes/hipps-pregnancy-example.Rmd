---
title: "Developing a real pipeline locally: the HIPPS pregnancy algorithm"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Developing a real pipeline locally: the HIPPS pregnancy algorithm}
...
This vignette shows how to develop a substantial, real-world All of Us analysis
against a local mock database. The example is the **HIPPS pregnancy algorithm**
([louisahsmith/allofus-pregnancy](https://github.com/louisahsmith/allofus-pregnancy)),
which identifies pregnancy episodes from OMOP clinical-event tables. We run its
unmodified algorithm code on a local DuckDB database built by `mockallofus`.

The synthetic data is random and does **not** resemble real All of Us data;
the goal is to exercise the code, not to produce meaningful results.

The general recipe is:

1. Connect to a mock database with `mock_aou_connect()`.
2. **Seed** the data your analysis looks for, so its queries return rows.
3. Run your analysis code exactly as you would on the Workbench.

## Seeding: the building blocks

A freshly built mock database contains synthetic participants and a broad spread
of generic events, but it won't contain *your study's* specific concepts. The
two exported primitives below let you put exactly the data your code needs into
the database:

- `mock_add_concepts()` --- add rows to the `concept` table (so concept-name
  searches and `concept_id` joins find them).
- `mock_add_occurrences()` --- add clinical-event rows to a domain table
  (`condition_occurrence`, `measurement`, ...), automatically generating
  surrogate keys and the matching `_ext` source-tracking rows.

```{r demo, eval = requireNamespace("duckdb", quietly = TRUE)}
library(mockallofus)
...
`mock_add_occurrences()` recycles `concept_id` and `date`, so you can pass a
vector per participant to build up temporally structured data --- which is what
the pregnancy example below does. `source = "ehr"` tags the `_ext` row so that
EHR-based functions (such as `allofus::aou_observation_period()`) see the event.

For coarser, prevalence-based seeding there are also `mock_seed_concept_set()`
and `mock_seed_survey()`, which are built on these same primitives.
...
The remaining code reproduces the project's `code/01_run-hipps.qmd` against the
mock database. It is **not executed when this vignette is built** (it downloads
the algorithm and code lists from GitHub and takes a little time), but it is
complete and runnable --- copy it into a script and run it.

Only three things differ from running on the Workbench, all marked below:

1. `mock_aou_connect()` replaces `aou_connect()`.
2. `seed_mock_pregnancies()` inserts synthetic pregnancies (the mock database
   has none by default).
3. The final `aou_workspace_to_bucket()` (Google Cloud Storage) step is dropped.

```{r setup-full, eval = FALSE}
library(tidyverse)
library(allofus)     # needs a version with backend-aware query execution
library(mockallofus)
...
This function is the part you would adapt for your own study. It is the same
pattern as the simple demo above, just richer: it builds *temporally coherent*
pregnancies so the algorithm's date logic (gestational windows, outcome timing)
has something to find. Read the comments as a template for manufacturing
structured data for a different project.

```{r seed-fn, eval = FALSE}
...
From here the code is identical to `01_run-hipps.qmd` --- the same `tbl()`
references, the same `aou_create_temp_table()` / `aou_compute()` calls, and the
same sourced algorithm functions.

```{r run-full, eval = FALSE}
...
Running this end-to-end touches most of what a complex All of Us analysis needs,
all of which works locally: `tbl()` + dplyr/dbplyr, `aou_create_temp_table()`,
`aou_compute()`, and BigQuery idioms such as the three-argument
`date_diff(a, b, sql("day"))`. The only Workbench-specific piece is the final
bucket upload, which has no local equivalent.
